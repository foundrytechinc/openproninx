...
void userinit(void) {
  ...
  p->cwd = 0; // Will be set in forkret after FS detection
...
pid_t wait(void) { return waitpid(-1, 0); }

// Wait for a direct child. target -1 selects any child; nohang returns 0
// immediately when matching children are still running.
pid_t waitpid(pid_t target, int nohang) {
  ...
  for (;;) {
    ...
    havekids = 0;
    for (p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
      if (p->parent != curproc || (target != -1 && p->pid != target))
        continue;
      havekids = 1;
      if (p->state == ZOMBIE) {
        // Found one.
...
// Return the lowest-numbered live process whose PID is greater than after.
// This keeps the process table private and avoids holding its lock while a
// system call copies records into user memory.
int procinfo_next(pid_t after, struct procinfo *out) {
  ...
  acquire(&ptable.lock);
  for (p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
    if (p->state == UNUSED || p->pid <= after)
      continue;
    if (found == 0 || p->pid < found->pid)
      found = p;
  }
  if (found) {
    out->pid = (pointer_t)found->pid;
    out->state = (proc_state_t)found->state;
    safestrcpy(out->name, found->name, sizeof(out->name));
    out->memory_size = (pointer_t)found->sz;
  }
  release(&ptable.lock);

  return found ? (int)out->pid : 0;
}
